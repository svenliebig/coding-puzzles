...
func TestCleanup(t *testing.T) {
	t.Run("t1", func(t *testing.T) {
		res := cleanup([]int{1, 1, 1})
		if len(res) != 1 && res[0] != 1 {
			t.Errorf("expected result to be [1] but was %v", res)
		}
	})
}

func BenchmarkCleanup_map(b *testing.B) {
	for n := 0; n < b.N; n++ {
		cleanup([]int{1, 1, 1})
	}
}
...
func BenchmarkSortRange(b *testing.B) {
	for n := 0; n < b.N; n++ {
		sortRange([]int{9, 1, 3, 4, 5, 1, 4, 6, 8, 3, 6, 5, 9, 3, 1, 1})
	}
}

func BenchmarkSortFor(b *testing.B) {
	for n := 0; n < b.N; n++ {
		sortFor([]int{9, 1, 3, 4, 5, 1, 4, 6, 8, 3, 6, 5, 9, 3, 1, 1})
	}
}
